...
export function ModelComparison({
  ...
  const bestTCO = Math.min(
    ...allModels.filter((m) => calculateTCO(m)).map((m) => calculateTCO(m)!)
  );
  const bestLifespan = Math.max(...allModels.map((m) => m.expected_lifespan_years || 10));
  ...
  const renderSpecRow = (config: SpecRowConfig) => {
    ...
    if (conditional && !allModels.some((m) => m[key] !== null && m[key] !== undefined)) {
      return null;
    }
        ...
        <div className="min-w-[800px]">
          <table className="w-full" style={{ borderCollapse: 'collapse', borderSpacing: 0 }}>
            ...
            <tbody>
              ...
              {/* Savings compared to selected (Calculated - Read Only) */}
